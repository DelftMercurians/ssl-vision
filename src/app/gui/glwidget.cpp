...
void GLWidget::paintGL() { mainDraw(); }

GLWidget::GLWidget(QWidget *parent, bool allow_qpainter_overlay)
    : QGLWidget(allow_qpainter_overlay ? QGLFormat(QGL::SampleBuffers) : QGLFormat(), parent) {
...
void GLWidget::resizeGL(int width, int height) { setupViewPort(width, height); }
...
void GLWidget::callHelp() {
  QMessageBox::information(
      this, "Keyboard and Mouse shortcuts",
      "Left-Click: Pick RGB Color\nShift-Modifier: Erase pixel from LUT\nShift-Ctrl-Modifier: Add pixel to "
...
void GLWidget::saveImage() {
  ...
  if (temp.getWidth() > 1 && temp.getHeight() > 1) {
    QFileDialog dialog(this, "Export image to file...", "", "PNG (*.png)");
