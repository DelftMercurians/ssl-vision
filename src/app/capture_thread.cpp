...
CaptureThread::CaptureThread(int cam_id) {
  ...
  control->addChild(
      (VarType *)(captureModule = new VarStringEnum("Capture Module", camId < 1 ? "Read from files" : "None")));
...
#ifdef DAHENG
  captureModule->addItem("Daheng GigE");
  daheng = new VarList("Daheng GigE");
...
void CaptureThread::setAffinityManager(AffinityManager *_affinity) { affinity = _affinity; }
...
VarList *CaptureThread::getSettings() { return settings; }
...
void CaptureThread::setFrameBuffer(FrameBuffer *_rb) { rb = _rb; }

FrameBuffer *CaptureThread::getFrameBuffer() const { return rb; }

VisionStack *CaptureThread::getStack() const { return stack; }
      ...
      if ((capture != nullptr) && (capture->isCapturing())) {
        ...
        if (bSuccess) {  // only on a good frame read do we proceed
          ...
          if (c_print_timings->getBool()) {
            ...
            std::cout << std::setw(13) << std::left << "getFrame" << std::setw(5) << std::right
                      << getFrame_duration.count() << " μs" << std::endl;
            auto convert_duration = std::chrono::duration_cast<std::chrono::microseconds>(t_convert - t_getFrame);
            std::cout << std::setw(13) << std::left << "copy&convert" << std::setw(5) << std::right
                      << convert_duration.count() << " μs" << std::endl;
            auto process_duration = std::chrono::duration_cast<std::chrono::microseconds>(t_process - t_convert);
            std::cout << std::setw(13) << std::left << "process" << std::setw(5) << std::right
                      << process_duration.count() << " μs" << std::endl;
            auto total_duration = std::chrono::duration_cast<std::chrono::microseconds>(t_process - t_start);
            std::cout << std::setw(13) << std::left << "total" << std::setw(5) << std::right << total_duration.count()
                      << " μs" << std::endl
                      << std::endl;
          }
