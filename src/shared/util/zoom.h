...
class Zoom {
 ...
 public:
  ...
  double getPanX() { return zoomCenterX; }
  double getPanY() { return zoomCenterY; }
  void reset() {
    flipX = 1.0;
    flipY = 1.0;
    zoomFactor = 1.0;
  ...
  bool getFlipX() { return (flipX < 0); }
  bool getFlipY() { return (flipY < 0); }
  double getFlipXval() { return flipX; }
  double getFlipYval() { return flipY; }
  ...
  void panLeft(double distance = DEFAULT_PAN_DISTANCE) { setPan(getPanX() - (distance * flipX), getPanY()); }
  void panRight(double distance = DEFAULT_PAN_DISTANCE) { setPan(getPanX() + (distance * flipX), getPanY()); }
  void panUp(double distance = DEFAULT_PAN_DISTANCE) { setPan(getPanX(), getPanY() - (distance * flipY)); }
  void panDown(double distance = DEFAULT_PAN_DISTANCE) { setPan(getPanX(), getPanY() + (distance * flipY)); }
  void zoomIn(double factor = DEFAULT_ZOOM_INCREMENT) { setZoom(getZoom() * factor); }
  void zoomOut(double factor = DEFAULT_ZOOM_INCREMENT) { setZoom(getZoom() / factor); }
  Zoom() {
    CENTER_ORIGIN = false;
  ...
  pixelloc zoom(int x, int y) {
    ...
    if (CENTER_ORIGIN) {
      ...
      result.x =
          (int)(((tx - (img_center_x) + (zoomCenterX * SURFACE_WIDTH)) * (zoomFactor * flipX)) + SURFACE_WIDTH / 2.0);
      result.y =
          (int)(((ty - (img_center_y) + (zoomCenterY * SURFACE_HEIGHT)) * (zoomFactor * flipY)) + SURFACE_HEIGHT / 2.0);
    }
  ...
  pixelloc invZoom(int x, int y, bool disable_center_origin = false) {
    ...
    result.y = (int)((((double)ty / (zoomFactor * flipY)) - (zoomCenterY * SURFACE_HEIGHT) + (img_center_y)) -
                     (getFlipY() ? 0.0 : 0.5));
    return result;
  }
};
